...
export const TOOLS = [
  {
    name: 'web_search',
    description:
      'Search the web and get back a short list of results (title, URL, snippet). Prefer this over opening a search engine in a tab: it is far cheaper, much faster, and does not disturb the user\'s tabs. Use it for anything you do not already know or cannot read on the current page — current facts, prices, news, documentation, finding the right site. Results are summaries: to read a page in full, follow with fetch_url.',
    parameters: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'What to search for.' },
        max_results: { type: 'integer', description: 'How many results to return (1-15, default 5).' }
      },
      required: ['query']
    }
  },
  {
    name: 'fetch_url',
    description:
      'Read the text of a web page WITHOUT opening a tab. Cheap and fast, and it leaves the user\'s tabs untouched. Use it to read a result from web_search or any address you already know. It fetches anonymously, so pages that need the user to be logged in will not work: for those, open the page in a tab and read it with get_page_text.',
    parameters: {
      type: 'object',
      properties: {
        url: { type: 'string', description: 'Full address, starting with https://' },
        max_chars: { type: 'integer', description: 'Maximum characters to return (default 8000, max 40000).' }
      },
      required: ['url']
    }
  },
  ...
  {
    name: 'navigate',
    description: 'Navigate the current tab to a URL and wait for it to load. Use full URLs (https://...). WARNING: this discards anything typed into the current page; if the tab has a partially filled form or draft, use open_tab instead. If you only need to READ the page, call fetch_url instead of navigating anywhere.',
    parameters: {
      type: 'object',
  ...
  {
    name: 'open_tab',
    description: 'Open a new tab (optionally at a URL) and make it the working tab. Only for pages you need to ACT on or that need the user to be logged in. If you just want to READ a page, call fetch_url instead: it is far cheaper and does not touch the user\'s tabs.',
...
export function toolsFor({ vision = true, debugger: dbg = true, history = true, downloads = true, js = true, webSearch = false, fetchUrl = true } = {}) {
  return TOOLS.filter((t) => {
    ...
    if (!webSearch && t.name === 'web_search') return false;
    if (!fetchUrl && t.name === 'fetch_url') return false;
    return true;
  });
}
