...
function ensureCdpListeners() {
  ...
  chrome.debugger.onDetach.addListener((source) => {
    ...
    cdpBuffers.delete(source.tabId);
  });
  if (chrome.tabs?.onRemoved) {
    chrome.tabs.onRemoved.addListener((tabId) => {
      cdpAttached.delete(tabId);
      cdpBuffers.delete(tabId);
    });
  }
}
...
export class BrowserController {
  ...
  // A lista de bloqueio vale para ler e para agir: um site proibido não pode
  // ter seu conteúdo enviado ao modelo, nem só o layout por captura de tela.
  async assertTabAllowed() {
    ...
    try {
      ...
      if (/Ação bloqueada/.test(String(e?.message))) throw e;
    }
  }
  ...
  async exec(func, args = [], world = 'ISOLATED') {
    const tabId = await this.ensureTab();
    if (!this.skipGuard) await this.assertTabAllowed();
    try {
  ...
  async dispose() {
    this.skipGuard = true; // limpar overlay não é leitura de conteúdo
    try {
  ...
  // Desanexa o depurador de forma síncrona, para quando o painel é fechado
  // no meio de uma tarefa e o finally assíncrono não chega a rodar.
  detachNow() {
    if (!HAS_CHROME || !chrome.debugger) return;
    for (const tabId of [...cdpAttached]) {
      try {
