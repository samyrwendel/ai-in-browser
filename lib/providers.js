...
async function httpError(res, conn) {
  ...
  const e = new Error(errorMessageFor(res.status, msg, conn));
  e.status = res.status;
  e.raw = msg;
  return e;
}

// Hosts oficiais de cada tipo de conexão. Quando a URL base aponta para outro
// lugar — usar o MiniMax pelo endpoint compatível com a Anthropic, por exemplo —
// o nome da conexão sozinho engana: a mensagem mandava adicionar saldo na
// Anthropic quando quem recusou foi a MiniMax. Por isso o host entra no texto.
const HOST_OFICIAL = { anthropic: 'api.anthropic.com', openrouter: 'openrouter.ai', openai: 'api.openai.com' };

function nomeDoProvedor(conn) {
  ...
  const oficial = HOST_OFICIAL[conn?.type];
  if (!host || !oficial || host === oficial || host.endsWith('.' + oficial)) return name;
  return `${name} (${host})`;
}

export function errorMessageFor(status, msg, conn) {
  const name = nomeDoProvedor(conn);
  ...
  if (status === 402) return `Créditos insuficientes em ${name}. Adicione saldo na conta desse provedor, e confira se a chave é a do plano certo. (${msg})`;
...
// Os 3 segundos das conexões "locais" existem para a detecção automática não
// travar quando o Ollama simplesmente não está instalado em localhost: ou algo
// responde na hora, ou não há nada ali. Esse raciocínio não vale quando o
// usuário aponta a mesma conexão para outra máquina — por Tailscale, por
// exemplo — onde a latência depende da rede e da rota. Por isso o prazo curto
// passa a valer só para endereços de loopback de verdade.
export function isLoopbackUrl(baseUrl) {
  try {
    const h = new URL(String(baseUrl)).hostname.toLowerCase().replace(/^\[|\]$/g, '');
    return h === 'localhost' || h === '::1' || h.endsWith('.localhost') || /^127\./.test(h) || /^0\.0\.0\.0$/.test(h);
  } catch {
    return false;
  }
}

export function modelsTimeout(conn) {
  if (conn?.local && isLoopbackUrl(conn?.baseUrl)) return 3000;
  return conn?.local ? 15000 : 20000;
}

export async function fetchModels(conn, opts = {}) {
  const timeoutMs = opts.timeoutMs ?? modelsTimeout(conn);
  ...
  try {
    ...
    // Ollama expõe as capacidades reais (visão, ferramentas, raciocínio) e o
    // contexto no endpoint nativo /api/tags, que o /v1/models não traz. Só o
    // Ollama tem esse endpoint; em qualquer outro ele falha e é ignorado.
    if (conn.local) list = await enrichOllamaCaps(conn, list, signal);
...
// Lê /api/tags do Ollama e cola as capacidades em cada modelo pelo nome. Falha
// silenciosa: um endpoint que não é Ollama (LM Studio, outro compatível) devolve
// 404/erro e a lista segue como estava.
async function enrichOllamaCaps(conn, list, signal) {
  try {
    const base = String(conn.baseUrl || '').replace(/\/v\d+\/?$/, '').replace(/\/+$/, '');
    const res = await fetch(base + '/api/tags', { signal });
    if (!res.ok) return list;
    const j = await res.json();
    const byName = new Map();
    for (const t of j.models || []) {
      const caps = Array.isArray(t.capabilities) ? t.capabilities : [];
      byName.set(t.name || t.model, {
        vision: caps.includes('vision'),
        tools: caps.includes('tools'),
        reasoning: caps.includes('thinking') || caps.includes('reasoning'),
        context: Number(t?.details?.context_length) || 0
      });
    }
    if (!byName.size) return list;
    return list.map((m) => {
      const c = byName.get(m.id);
      if (!c) return m;
      return { ...m, caps: { vision: c.vision, tools: c.tools, reasoning: c.reasoning }, context: c.context || m.context };
    });
  } catch {
    return list;
  }
...
async function* openaiStream(conn, { model, messages, system, temperature, maxTokens, signal, stream = true, effort }) {
  ...
  for await (const ev of sseEvents(res.body, signal)) {
    ...
    if (j.error) throw new Error(errorMessageFor(j.error.code, j.error.message || JSON.stringify(j.error), conn));
...
export function modelSupportsVision(conn, m) {
  ...
  if (m?.caps && typeof m.caps.vision === 'boolean') return m.caps.vision; // capacidade real (Ollama)
...
export function modelLikelySupportsTools(conn, m) {
  const id = (m?.id || '').toLowerCase();
  if (m?.caps && typeof m.caps.tools === 'boolean') return m.caps.tools; // capacidade real (Ollama)
  if (conn.type === 'openrouter' || conn.type === 'anthropic' || isOfficialOpenAI(conn)) return true;
  // Endpoint remoto personalizado — um gateway de nuvem como o drael.sh, não um
  // servidor local: assume que suporta tool calling. Serviços remotos pagos
  // quase sempre suportam, e o nome do modelo (drael-v1, etc.) não segue as
  // convenções que a lista abaixo reconhece. Se este servidor recusar as
  // ferramentas, o agente cai sozinho para o protocolo JSON (TOOLS_UNSUPPORTED).
  // Sem isto, a cooperação trocava o modelo escolhido pelo usuário por outro só
  // porque o nome não estava na lista.
  if (!conn.local) return true;
  // Servidores locais (Ollama/LM Studio) rodam modelos pequenos que muitas vezes
  // não têm tool calling; aí o nome ainda é o melhor palpite disponível.
...
export async function chatWithTools(conn, { model, messages, system, tools, temperature, maxTokens, signal, effort, vision = true }) {
  ...
  if (!res.ok) throw classifyToolError(await httpError(res, conn));
  const j = await res.json();
  if (j.error) throw classifyToolError(Object.assign(new Error(errorMessageFor(j.error.code, j.error.message || JSON.stringify(j.error), conn)), { status: j.error.code, raw: j.error.message }));
  const msg = j.choices?.[0]?.message || {};
