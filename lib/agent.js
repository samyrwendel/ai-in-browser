...
export function buildSystemPrompt({ maxSteps, jsonMode, tools, userSystem, tabInfo, homeTab, vision, visionHelperName }) {
  const lines = [
    ...
    (tools || []).some((t) => t.name === 'web_search')
      ? '- To find something you do not know and cannot read on the current page, call web_search FIRST. Do not open a search engine in a tab for it: web_search costs a fraction of the tokens, takes a fraction of the time and leaves the user\'s tabs alone. To read one of those results, use fetch_url rather than opening it. Open a tab only when you must act on the page (click, type, submit) or when the page needs the user to be logged in.'
      : '- You have no search tool: to look something up you must open a search engine in a tab. Prefer opening a NEW tab when the current one has work in progress.',
