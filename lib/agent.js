...
export function buildSystemPrompt({ maxSteps, jsonMode, tools, userSystem, tabInfo, vision, visionHelperName }) {
  const lines = [
...
export class AgentRunner {
  constructor({ conn, model, modelInfo, browser, settings, onEvent, perms, visionHelper }) {
    this.conn = conn;
    this.model = model;
  ...
  async callModel(tabInfo) {
    ...
    const system = buildSystemPrompt({ maxSteps: this.maxSteps, jsonMode: this.mode === 'json', tools, userSystem: (s.agent?.systemPrompt || '').trim(), tabInfo, vision: this.vision, visionHelperName: this.visionHelper?.name });
    ...
    switch (name) {
      ...
      case 'screenshot': {
        ...
        try {
          ...
          return { text: `${r.text}\n\nNão foi possível descrever a captura com ${h.name}: ${e?.message || e}. Use get_page_state.`, image: r.image };
        }
      }
      case 'evaluate_js':
