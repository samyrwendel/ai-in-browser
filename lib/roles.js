...
export function pinnedProblem(cfg, settings) {
  if (!cfg || cfg.mode !== 'pinned' || !cfg.modelId) return null;
  const conn = (settings.connections || []).find((c) => c.id === cfg.connectionId);
  if (!conn) return 'o provedor não existe mais';
  if (conn.enabled !== true) return `o provedor ${conn.name} está desativado em Conexões`;
  const m = { id: cfg.modelId };
  if (isRouter(m)) return 'roteadores não servem para delegar uma habilidade';
  if (/:batch$/i.test(cfg.modelId)) return 'variantes :batch são lotes assíncronos e não respondem em tempo real';
  if (cfg.modelId.startsWith('~')) return 'apelidos ~…latest apontam para outro modelo e não são estáveis';
  return null;
}

export function isNonInteractive(m) {
  const id = String(m?.id || '');
  return /:batch$/i.test(id) || id.startsWith('~');
}

export function isRouter(m) {
...
export function capabilities(conn, m) {
  ...
  const reasoning = m?.caps && typeof m.caps.reasoning === 'boolean'
    ? m.caps.reasoning
    : isOR && params.length ? params.includes('reasoning') || params.includes('include_reasoning') : REASONING_RE.test(id);
...
export function resolveRole(roleId, settings, modelsByConn, main) {
  ...
  if (cfg.mode === 'pinned' && cfg.connectionId && cfg.modelId) {
    const conn = (settings.connections || []).find((c) => c.id === cfg.connectionId);
    const pinnedM = { id: cfg.modelId };
    // fixação inválida cai no automático: provedor desativado, roteador,
    // lote assíncrono (:batch) ou apelido dinâmico (~…latest)
    const usable = conn && conn.enabled === true && !isRouter(pinnedM) && !isNonInteractive(pinnedM);
    if (usable) {
      const m = (modelsByConn[conn.id] || []).find((x) => x.id === cfg.modelId) || pinnedM;
...
// Encadeia o sinal de quem chamou com um tempo limite próprio: um ajudante
// que não responde não pode travar a conversa nem a tarefa do agente.
function withDeadline(signal, ms) {
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(new DOMException('tempo limite', 'TimeoutError')), ms);
  ...
  return {
    ...
    expirou: () => ctrl.signal.aborted && !signal?.aborted
  };
}

export class HelperTimeout extends Error {
  constructor(nome, ms) {
    super(`${nome} não respondeu em ${Math.round(ms / 1000)}s`);
    this.name = 'HelperTimeout';
  }
}

export async function describeImages(conn, modelId, images, { purpose = 'chat', question = '', signal, maxTokens = 1500, timeoutMs = 90000 } = {}) {
  ...
  const d = withDeadline(signal, timeoutMs);
  try {
    ...
    if (d.expirou()) throw new HelperTimeout(modelId, timeoutMs);
    throw e;
  } finally {
    d.clear();
  }
...
export async function generateTitle(conn, modelId, text, { signal, timeoutMs = 25000 } = {}) {
  const d = withDeadline(signal, timeoutMs);
  try {
  const r = await P.chatWithTools(conn, {
    model: modelId,
