...
function ensureCdpListeners() {
  // registrado sob demanda: a permissão "debugger" é opcional e pode ser
  // concedida depois do carregamento desta página.
...
export class BrowserController {
  ...
  async pickTab(preferredId) {
    const ok = (t) => t && /^(https?|file):/i.test(t.url || '');
    if (preferredId != null) {
      try {
  ...
  async init(preferredId) {
    ...
    if (!t) t = await chrome.tabs.create({ url: 'about:blank', active: true });
    this.tabId = t.id;
  ...
  async getState({ includeText = true } = {}) {
    ...
    return formatState(s, includeText);
  }
