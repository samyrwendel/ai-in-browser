...
const TIERS = {
  vision: [
    [/claude-(opus|sonnet)-(4|5)|claude-fable|(^|\/)gpt-5(?!-nano|-mini)|gemini-(2\.5|3)-pro|(^|\/)o3(?!-mini)/i, 3],
    [/claude-haiku-4|gpt-5-mini|gpt-4\.1(?!-nano)|gpt-4o(?!-mini)|gemini-2\.5-flash|gemini-3-flash|llama-4-maverick|qwen3-vl|qwen2\.5-vl|pixtral-large|grok-4(?!-fast)|mistral-medium-3|llava|gemma3|minicpm-v/i, 2]
  ],
  tools: [
    [/claude-(opus|sonnet)-(4|5)|claude-fable|(^|\/)gpt-5(?!-nano)|gpt-4\.1(?!-nano)|gemini-(2\.5|3)-pro|kimi-k2|qwen3-(235b|coder|max)|glm-4\.[5-9](?!-flash|-air)|grok-4(?!-fast)|deepseek-(chat-)?v3/i, 3],
    [/gpt-4o|gemini-2\.5-flash|claude-haiku|llama-4|qwen3|mistral-medium|grok-4-fast|devstral|gpt-oss|llama3\.[1-3]|llama-3\.[1-3].*70b|hermes|command-r/i, 2]
  ],
  reasoning: [
    [/(^|\/)o3(?!-mini)|(^|\/)gpt-5(?!-nano|-mini)|claude-opus-(4|5)|claude-sonnet-(4\.5|4\.6|5)|claude-fable|gemini-(2\.5|3)-pro|deepseek-r1(?!-distill)|grok-4(?!-fast)|qwen3-max|kimi-k2-thinking/i, 3],
    [/o4-mini|o3-mini|gpt-5-mini|deepseek-r1-distill|qwq|qwen3-(235b|32b)|magistral|glm-4\.[5-9]|gemini-2\.5-flash|phi-4-reasoning|gpt-oss|deepseek-r1:/i, 2]
  ]
};
...
export function qualityTier(roleId, cap, m) {
  ...
  for (const [re, tier] of TIERS[key] || []) if (re.test(id)) return tier;
  return 1;
}
...
export function candidates(roleId, settings, modelsByConn, main, prefer) {
  ...
  for (const conn of (settings.connections || []).filter((c) => c.enabled === true)) {
    for (const m of modelsByConn[conn.id] || []) {
      if (isRouter(m)) continue; // roteadores não são um modelo previsível para delegar
  ...
  out.sort((a, b) => b.score - a.score || b.tier - a.tier || (a.price ?? 99) - (b.price ?? 99) || a.name.localeCompare(b.name));
