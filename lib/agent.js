...
export function buildSystemPrompt({ maxSteps, jsonMode, tools, userSystem, tabInfo, vision }) {
  const lines = [
    ...
    vision ? '- Prefer get_page_state; use screenshot when layout or images matter (maps, charts, canvas, image-only buttons) or when the DOM is confusing.' : '- This model has no vision: rely on get_page_state, get_page_text, find_elements and get_page_html.',
    '- If an action fails or nothing changes, try another approach: scroll, find_elements, a different element, the site search, or navigate directly to a known URL (e.g. https://www.google.com/search?q=...).',
  ...
  if (tabInfo) lines.push('', `Current working tab: "${tabInfo.title || ''}" ${tabInfo.url || ''}`);
...
export class AgentRunner {
  constructor({ conn, model, modelInfo, browser, settings, onEvent, perms }) {
    this.conn = conn;
    this.model = model;
  ...
  async callModel(tabInfo) {
    ...
    const system = buildSystemPrompt({ maxSteps: this.maxSteps, jsonMode: this.mode === 'json', tools, userSystem: (s.agent?.systemPrompt || '').trim(), tabInfo, vision: this.vision });
  ...
  async execTool(name, args) {
    ...
    switch (name) {
      ...
      case 'screenshot': {
        if (!this.vision) return { text: 'O modelo atual não suporta imagens; use get_page_state.' };
  ...
  async run(task, { history = [], context = null } = {}) {
    this.maxSteps = Math.max(3, Math.min(200, Number(this.settings.agent?.maxSteps) || 25));
    this.aborted = false;
    const t0 = Date.now();
    ...
    try {
      ...
      while (!this.aborted && steps < this.maxSteps) {
        const tabInfo = await this.browser.currentInfo?.();
        this.emit('status', { text: `Pensando… (passo ${steps + 1}/${this.maxSteps})`, tab: tabInfo });
        let res;
        try {
    ...
    const result = { ...finished, steps: this.steps, usage: this.usage, ms: Date.now() - t0, mode: this.mode, vision: this.vision };
