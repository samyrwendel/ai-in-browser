...
function friendlyError(status, msg, conn) {
  ...
  if (status === 403 && !conn?.apiKey) {
    return `${name} recusou a origem da extensão (HTTP 403). No Ollama, defina OLLAMA_ORIGINS="chrome-extension://*" na máquina onde ele roda e reinicie o serviço.`;
  }
  ...
  try {
    ...
    if (conn.type !== 'anthropic' && !conn.apiKey) {
      ...
      if (status === 403) {
        ...
        result.message = `A listagem de modelos funciona, mas o envio de mensagens é recusado (HTTP 403): o servidor não aceita a origem da extensão. No Ollama, defina OLLAMA_ORIGINS="chrome-extension://*" na máquina onde ele roda e reinicie o serviço.`;
        return result;
      }
    }
...
export function modelSupportsVision(conn, m) {
  ...
  return /llava|vision|-vl|vl-|gemma3|gemma-3|minicpm-v|moondream|pixtral|bakllava|qwen2\.5vl|qwen3-vl|llama3\.2-vision|llama-3\.2-.*vision|mistral-small3|granite3\.2-vision|internvl|smolvlm|gpt-4|claude|gemini/.test(id);
}
